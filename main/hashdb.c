...
static inline uint64_t sha1hash(const char* v) {
    unsigned char result[SHA_DIGEST_LENGTH];
    uint64_t* k = (uint64_t*)result;
    SHA1(v, strlen(v), result);
    return *k;
}
...
DB* ht_open(const char* filename, size_t initial_capacity) {
    ...
    dbh->hash = murmur3_32;
    dbh->hash2 = murmur3_32;
    //dbh->hash = murmur_32_scramble; //cnange!!!! to md5hash
    //dbh->hash2 = rot1333; 
...
int ht_set(DB* db, const char* key, const char* value) {
    ...
    if ( _ht_search(db, &cur, key) )
        ...
        // элемент не найден
        if ( cur.th.nodes*100 >= cur.th.capacity*FILL_FACTOR) {
